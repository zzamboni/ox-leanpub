...
If you have any feedback or bug reports, please open an issue at https://github.com/zzamboni/ox-leanpub/issues.
...
By default, the =ox-leanpub= module sets things up for exporting books in Markua format. For example, using =use-package=:
...
If you want to export your books in LFM format, you need to additionally load the =ox-leanpub-markdown= exporter and tell =ox-leanpub-book= to set up the corresponding menu entries, as follows:
...
Depending on whether you load the Markua or Markdown exporter, you will see two new sections in Org's export menu (~C-c C-e~), called "Export to Leanpub Markua" and "Export to Leanpub Markdown", with options to export to a file or a temporary buffer. Additionally, the =ox-leanpub-book= module adds three items in the corresponding section:
...
- "Book: Current chapter" to explicitly export only the current chapter to its own file. This also updates =Subset.txt=, so it can be used to preview the current chapter.
...
- If the heading has an =EXPORT_FILE_NAME= property, it is used (note: this filename should already specify the output directory, e.g. =manuscript/chapter.markua=), unless the =#+LEANPUB_BOOK_RECOMPUTE_FILENAMES= file property is set.
- If the heading has a =NAME=, =ID= or =CUSTOM_ID= property, it is used as the base filename, and used to construct the filename inside =manuscript/=. The final filename is stored in the =EXPORT_FILE_NAME= property.
- Othewise, the filename is generated based on the heading title by lowercasing it and replacing all non-alphanumeric characters with hypens. The final filename is likewise stored in =EXPORT_FILE_NAME=.
